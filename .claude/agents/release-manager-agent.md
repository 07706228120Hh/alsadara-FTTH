...
- **الإصدار الحالي فعلياً**: `2.3.4+309` (منشور، latest) — لا `2.2.25+304`.
- **CI مقفل بالفوترة** ⇒ الإصدارات **يدوية** عبر `gh release create --latest`.
- **v2.3.4 = أول إصدار بنشر باكند مصاحب** للتطبيق (سابقاً Flutter فقط) — أدرج بوابة تحقّق نشر الباكند (SCP + restart) في checklist عند تكرار ذلك.
- **تحذير حرج**: `origin` = مستودع الإصدارات ⇒ دفع tag `v*` = **إصدار إنتاج حقيقي يصل كل الأجهزة تلقائياً** (يتطلب موافقة بشرية صريحة).
- أرقام الإصدار تُزامَن **يدوياً** بين `pubspec.yaml` و`installer.iss` — تحقّق من تطابقهما قبل أي إصدار.
- **درس EF vs psql**: أي إصدار يمسّ SQL خام في EF يحتاج تحقّقاً عبر EF لا psql (نسّق مع testing-qa).
