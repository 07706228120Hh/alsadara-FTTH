...
class HomePageTasksState extends State<HomePageTasks> {
  ...
  /// تطبيع اسم القسم للمقارنة العميلية — **بلا تجريد «ال» التعريف**.
  ///
  /// يجب أن تبقى مطابقة القسم في العميل متّسقة مع ما يُرسَل للخادم في
  /// `getSummary` (بارامتر `department`): الخادم يطابق قيمة القسم **الخام
  /// كما هي مخزّنة** (مثل «الحسابات» بأداة التعريف «ال»)، لذلك يجب ألّا يجرّد
  /// العميل «ال» وإلا اختلف الجانبان فتظهر أرقام صفرية لليدر قسمه «الحسابات».
  ///
  /// نكتفي بـ trim + دمج المسافات الداخلية (كما في [normalizeName]) لمنع سقوط
  /// المطابقة بسبب فروق المسافات فقط — دون أي تجريد لأداة التعريف.
  String _normalizeDept(String dept) {
    return dept.trim().replaceAll(RegExp(r'\s+'), ' ');
  }
