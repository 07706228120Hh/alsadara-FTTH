# ============================================================
# Build Complete Script for Alsadara v1.2.5
# ============================================================
...
Write-Host "    Build Complete - Alsadara v1.2.5" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    if (Test-Path "Distribution_v1.2.5") {
        Write-Host "  Removing old folder..." -ForegroundColor Gray
        Remove-Item "Distribution_v1.2.5" -Recurse -Force
    }
    New-Item -Path "Distribution_v1.2.5\Portable" -ItemType Directory -Force | Out-Null
    New-Item -Path "Distribution_v1.2.5\Installer" -ItemType Directory -Force | Out-Null
...
try {
    ...
    $destPath = "Distribution_v1.2.5\Portable\"
    ...
    if (-not (Test-Path "Distribution_v1.2.5\Portable\alsadara.exe")) {
...
if ($innoAvailable) {
    try {
        ...
        $issFile = "alsadara_installer_v1.2.5.iss"
        ...
        if ($LASTEXITCODE -eq 0) {
            if (Test-Path "Distribution_v1.2.5\Installer\Alsadara_v1.2.5_Setup.exe") {
...
try {
    $zipFile = "Alsadara_v1.2.5_Complete.zip"
    if (Test-Path $zipFile) {
        Remove-Item $zipFile -Force
    }
    
    Compress-Archive -Path "Distribution_v1.2.5" -DestinationPath $zipFile -Force
...
if (Test-Path "Distribution_v1.2.5\Portable\alsadara.exe") {
    $portableSize = [math]::Round((Get-ChildItem "Distribution_v1.2.5\Portable" -Recurse | Measure-Object -Property Length -Sum).Sum / 1MB, 2)
    Write-Host "  OK Portable Version:" -ForegroundColor Green
    Write-Host "     Path: Distribution_v1.2.5\Portable\" -ForegroundColor White
...
if (Test-Path "Distribution_v1.2.5\Installer\Alsadara_v1.2.5_Setup.exe") {
    $setupSize = [math]::Round((Get-Item "Distribution_v1.2.5\Installer\Alsadara_v1.2.5_Setup.exe").Length / 1MB, 2)
    Write-Host "  OK Setup Installer:" -ForegroundColor Green
    Write-Host "     Path: Distribution_v1.2.5\Installer\Alsadara_v1.2.5_Setup.exe" -ForegroundColor White
...
if (Test-Path "Alsadara_v1.2.5_Complete.zip") {
    $zipSize = [math]::Round((Get-Item "Alsadara_v1.2.5_Complete.zip").Length / 1MB, 2)
    Write-Host "  OK Compressed Archive:" -ForegroundColor Green
    Write-Host "     Path: Alsadara_v1.2.5_Complete.zip" -ForegroundColor White
...
if ($allSuccess -or (Test-Path "Distribution_v1.2.5\Portable\alsadara.exe")) {
    ...
    switch ($choice) {
        ...
        "2" {
            if (Test-Path "Distribution_v1.2.5\Portable\alsadara.exe") {
                ...
                Start-Process "Distribution_v1.2.5\Portable\alsadara.exe"
